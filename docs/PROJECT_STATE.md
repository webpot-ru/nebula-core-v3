...
- `translator_tts.py` localizes non-English `story_data.json` through VectorEngine Gemini, keeps translated display fields for storyboard/card rendering, prepares separate narration-only fields where raw URLs are spoken as localized on-screen-link phrases, then submits narration text to AI33 TTS v3. Channels can use separate `tts_voice` and `comment_tts_voice` roles; the script concatenates role segments into one `narration.mp3` and writes combined word timings for karaoke.
...
- The default narration order mirrors visible card text for karaoke alignment: title, body, then comment bodies. For raw URLs and service link lines, visible fields keep the actual link on the card while `narration_title`, `narration_body`, and `comments[].narration_body` say the localized "link on screen" phrase. `--include-comment-labels` can restore spoken "Comment by user" labels when strict word-level visual sync is not required.
- If `comment_tts_voice` differs from `tts_voice`, `translator_tts.py` now runs multi-voice TTS: title/body use the narrator voice, comment bodies use the comment voice, long text is chunked with `--tts-segment-max-chars`, FFmpeg concatenates the MP3 segments, and `narration.json` is rewritten with combined offset word timings when AI33 returns usable word timing data. The parser accepts explicit word timing arrays and ElevenLabs-style `alignment` / `normalized_alignment` character timing arrays, converting character timings into words for in-text karaoke. If AI33 returns audio but no usable timings, the script generates `timing_status=estimated` word timings from the exact segment text and MP3 duration so render-mode karaoke still highlights every visible word instead of failing or skipping words. Fully missing or partial unfillable timings still save sanitized payload-shape diagnostics, and `render.py` falls back to clean slide-progress frames with audio unless live `--require-karaoke` is active. Retryable AI33 task errors during per-segment multi-voice generation are retried before the pipeline fails.
- RedditSim karaoke highlights stable smart phrase groups instead of single words. Groups target 2-5 words and roughly 24-34 characters, stop at punctuation or line breaks, and never cross title/body/comment boundaries. This matches short-video caption rhythm and makes estimated timings less visibly off. The `.kw-active` styling no longer changes font weight, padding, display mode, transform, line-height, or letter spacing, so highlighted words do not reflow neighboring text between lines.
...
- `.github/workflows/auto_publish.yml` now supports manual topic strategy selection and uses `privacy_status=unlisted` by default for manual dispatch. Its default manual topic strategy is `channel_mix`, not forced `human_drama`, so `acc1` uses its dark curiosity / facts / internet-lore mix unless a family is explicitly selected. If a family is explicitly selected but is not present in the channel mix, `scraper.py` now uses that family's preset sources instead of falling back to the channel's legacy subreddit list. Scheduled cron entries should remain `unlisted` until one post-fix live artifact is reviewed for channel, language, audio, clean render, and karaoke quality. The workflow and `video_dry_run.yml` use `time_filter=auto` by default for topic-family windows and set `AI_QUALITY_FAIL_OPEN=0`, explicit `MAX_AI_CANDIDATES`, `STORY_SIMILARITY_THRESHOLD=0.72`, and `TOPIC_FATIGUE_LOOKBACK=10`.
