...
- Acc1 unlisted live run `28573148388` on 2026-07-02 succeeded through upload to `gZP4fiM5iTo` on `ChonkerTalks Россия` with `privacy=unlisted` and `language=ru`, but AI33 returned transcript metadata without complete word timings for a very long narrator segment, so `render.py` fell back to non-karaoke slide-progress before upload. Follow-up run `28574532627` stopped before upload because `auto_publish.yml` now requires karaoke; the active follow-up fix chunks long TTS narration and parses ElevenLabs character alignment into word timings for future `auto_publish.yml` uploads.
