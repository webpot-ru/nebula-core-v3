...
class Acc1GreenlightTemplateTests(unittest.TestCase):
    ...
    def test_template_is_deterministic_and_selects_exact_top_one(self):
        queue = self._queue()
        ...
        self.assertEqual(first["source"]["post_id"], review["top_topics"][0]["post_id"])
    ...
    def test_explicit_post_id_must_be_an_eligible_top_topic(self):
        ...
        review = self._review(queue)
        payload = greenlight_template.build_template(queue, review, post_id="candidate-1")
