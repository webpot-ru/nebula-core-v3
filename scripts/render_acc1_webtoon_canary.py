...
def resolve_storyboard(download_root: Path) -> Path:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"expected exactly one production webtoon storyboard, found {len(matches)}",
        )
    return matches[0]
...
def resolve_master_audio(download_root: Path) -> Path:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"expected exactly one narration-master.mp3, found {len(matches)}",
        )
    return matches[0]


def build_canary_storyboard(
    source: dict,
    *,
    scene_count: int = 4,
    ...
    start_index = next(
        (
            ...
            if scene.get("presentation") == "story"
        ),
        None,
    )
    ...
    if len(selected) != scene_count or any(
        scene.get("presentation") != "story" for scene in selected
    ):
        raise RuntimeError("source storyboard has no four consecutive story scenes")
    if len({
        (scene.get("motion") or {}).get("module")
    ...
    original_plan.update({
        ...
        "module_usage": {
            module: sum(
                (scene.get("motion") or {}).get("module") == module
...
def review_frame_times(
    ...
    *,
    ...
    return [
        round(
            float(scene["start_sec"]) + float(scene["duration_sec"]) * progress,
            3,
        )
...
def main() -> int:
    ...
    storyboard_path = (
        Path(args.storyboard_path).resolve()
        if args.storyboard_path
        else resolve_storyboard(download_root)
    )
    ...
    if args.storyboard_path:
        ...
        start = float(
            source.get(
                "canary_source_start_sec",
                source["slides"][0]["start_sec"],
            ),
        )
        end = float(
            source.get(
                "canary_source_end_sec",
                source["slides"][-1]["end_sec"],
            ),
        )
    else:
        storyboard, start, end = build_canary_storyboard(
            source,
    ...
    subprocess.run(
        [
            ...
            "-i", str(master), "-vn", "-c:a", "libmp3lame", "-q:a", "2",
            str(audio),
        ],
        check=True,
    )
    video = output / "webtoon-canary.mp4"
    report = render_chrome_guided_webtoon(
        storyboard,
        root,
        video,
        audio=audio,
