...
SAMPLE_TEXTS: dict[str, dict[str, dict[str, str]]] = {
    "horror_plain": {
        "ru": {
            "narrator": "Ночная смена начиналась совершенно обычно. Пустой коридор, холодный свет и список простых правил. Я почти смеялся над ними, пока часы не показали три пятнадцать. В этот момент из закрытого туалета раздался тихий стук. Один раз. Потом второй. Я замер, потому что последнее правило запрещало отвечать. Но за дверью уже произнесли моё имя.",
            "comment": "Я бы на твоём месте остановился прямо здесь. Слишком много деталей не сходится.",
        },
    },
    "neutral": {
        "ru": {
