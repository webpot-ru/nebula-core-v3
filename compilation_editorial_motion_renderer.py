...
def _render_segment_plan(
    scenes: list[dict[str, Any]], *, max_duration_sec: float = DEFAULT_SEGMENT_MAX_DURATION_SEC,
) -> list[dict[str, Any]]:
    """Split contiguous scenes without cutting a scene or changing its duration."""
    ...
    for index, group in enumerate(groups, start=1):
        ...
        plan.append({
            ...
            "duration_sec": round(source_end - source_start, 6),
...
def build_editorial_render_segment_plan(
    ...
    """Return the public, path-free plan used by GitHub's render matrix."""

    scenes = preflight_editorial_motion_storyboard(storyboard, Path(artifact_root).resolve())
...
def _probe_h264(path: Path, *, cwd: Path) -> dict[str, Any]:
    ...
    try:
        ...
        stream = next(item for item in payload.get("streams") or [] if item.get("codec_name"))
    ...
    if (
        ...
        or [stream.get("width"), stream.get("height")] != [CANVAS_WIDTH, CANVAS_HEIGHT]
    ):
...
def render_editorial_motion_segment(
    ...
    """Render one silent segment and discard its frame cache before returning."""
    ...
    with tempfile.TemporaryDirectory(prefix=f".hf-segment-{segment_index:03d}-", dir=root) as temp:
        workspace = Path(temp) / "workspace"
        _write_workspace(
            segment["scenes"], root, root / "episode-script.json",
            float(segment["duration_sec"]), style_profile=style_profile,
            workspace_path=workspace,
        )
        check = _run([*cli, "check", "--json"], cwd=workspace)
        _run([
            *cli, "render", "--quality", "high", "--workers", "1",
            "--output", str(output),
        ], cwd=workspace)
...
def assemble_editorial_motion_segments(
    ...
    """Concatenate verified silent segments and mux the existing master narration."""

    root = Path(artifact_root).resolve()
    scenes = preflight_editorial_motion_storyboard(storyboard, root)
    plan = _render_segment_plan(scenes, max_duration_sec=max_duration_sec)
    if len(segment_paths) != len(plan):
        raise EditorialMotionRenderError("segment file count does not match deterministic plan")
    ...
    for segment, raw_path in zip(plan, segment_paths, strict=True):
        ...
        if abs(float(probe["duration_sec"]) - float(segment["duration_sec"])) > 0.12:
            raise EditorialMotionRenderError("segment duration does not match deterministic plan")
    ...
    with tempfile.TemporaryDirectory(prefix=".hf-assembly-", dir=root) as temp:
        ...
        concat_list.write_text(
            "".join(f"file '{str(path).replace(chr(39), chr(39) + '\\\'' + chr(39))}'\n" for path in verified_paths),
            encoding="utf-8",
        )
        silent_output = temp_root / "joined-silent.mp4"
        _run([
            ffmpeg, "-y", "-f", "concat", "-safe", "0", "-i", str(concat_list),
            "-c", "copy", "-movflags", "+faststart", str(silent_output),
        ], cwd=temp_root)
        _run([
...
def render_editorial_motion_compilation(
    ...
    *,
    ...
    """Sequential compatibility wrapper around the bounded segment renderer."""
    ...
    with tempfile.TemporaryDirectory(prefix=".hf-sequential-segments-", dir=root) as temp:
        temp_root = Path(temp)
        segment_paths: list[Path] = []
        for item in plan["segments"]:
            index = int(item["index"])
            segment_path = temp_root / f"segment-{index:03d}.mp4"
            render_editorial_motion_segment(storyboard, root, index, segment_path)
            segment_paths.append(segment_path)
        return assemble_editorial_motion_segments(
            storyboard, root, segment_paths, output, audio=audio_path,
        )
